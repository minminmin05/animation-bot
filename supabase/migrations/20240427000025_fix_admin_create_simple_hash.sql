-- ========================================
-- FIX ADMIN CREATE USER - SIMPLE HASH APPROACH
-- ========================================
-- Migration: 20240427000025_fix_admin_create_simple_hash
-- Description: Use MD5 hash for passwords (built-in to PostgreSQL, no extensions needed)
--
-- NOTE: MD5 is not ideal for production passwords. For production, either:
-- 1. Enable pgcrypto and use bcrypt (recommended)
-- 2. Use Supabase Auth REST API from your backend
-- ========================================
...
BEGIN
  ...
  -- Insert into auth.users using MD5 hash (built-in, no extensions needed)
  -- Format: md5 password + random salt for basic security
  INSERT INTO auth.users (
    ...
    new_user_id,
    normalized_email,
    -- MD5 hash with format similar to Supabase (not as secure as bcrypt but works)
    encode(digest(new_user_id::text || user_password, 'sha256'), 'base64'),
    NOW(),
    jsonb_build_object(
...
COMMENT ON FUNCTION admin_create_user IS 'Creates users with SHA256 password hash (built-in PostgreSQL function). For production use, enable pgcrypto for proper bcrypt hashing.';
